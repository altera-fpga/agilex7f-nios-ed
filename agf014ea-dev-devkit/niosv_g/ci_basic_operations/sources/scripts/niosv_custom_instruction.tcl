...
# File: niosv_custom_instruction.tcl
# Generated on: Tue Oct 17 06:05:31 2023
...
if {[is_project_open]} {
	if {[string compare $quartus(project) "niosv_custom_instruction"]} {
		puts "Project niosv_custom_instruction is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists niosv_custom_instruction]} {
		project_open -revision niosv_custom_instruction niosv_custom_instruction
	} else {
		project_new -revision niosv_custom_instruction niosv_custom_instruction
	}
...
if {$make_assignments} {
	...
	set_global_assignment -name PROJECT_CREATION_TIME_DATE "11:40:13  MARCH 05, 2023"
	set_global_assignment -name LAST_QUARTUS_VERSION "23.4.0 Pro Edition"
	set_global_assignment -name SYSTEMVERILOG_FILE custom_pe.sv
	...
	set_global_assignment -name IP_FILE ip/sys/sys_custom_pe_2_0.ip
	set_global_assignment -name IP_FILE ip/sys/sys_custom_pe_1_0.ip
