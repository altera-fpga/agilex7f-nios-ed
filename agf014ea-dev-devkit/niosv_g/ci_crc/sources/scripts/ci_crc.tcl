...
# File: ci_crc.tcl
# Generated on: Tue Oct 17 06:17:15 2023
...
if {[is_project_open]} {
	if {[string compare $quartus(project) "ci_crc"]} {
		puts "Project ci_crc is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists ci_crc]} {
		project_open -revision ci_crc ci_crc
	} else {
		project_new -revision ci_crc ci_crc
	}
...
if {$make_assignments} {
	...
	set_global_assignment -name PROJECT_CREATION_TIME_DATE "02:17:48  MAY 03, 2023"
	set_global_assignment -name LAST_QUARTUS_VERSION "23.4.0 Pro Edition"
	set_global_assignment -name TCL_SCRIPT_FILE crc_custom_pe_hw.tcl
	...
	set_global_assignment -name IP_FILE ip/sys/sys_crc_custom_pe_1.ip
	...
	set_global_assignment -name IP_FILE ip/sys/sys_reset_bridge_1.ip
