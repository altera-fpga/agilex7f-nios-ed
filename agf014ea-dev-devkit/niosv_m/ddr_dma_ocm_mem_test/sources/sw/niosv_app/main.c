...
#define DATA_DESTINATION_BASE EMIF_FM_0_ARCH_BASE
#define DATA_SOURCE_BASE INTEL_ONCHIP_MEMORY_1_BASE
#define DDR4_EMIF_ARCH_BASE EMIF_FM_0_ARCH_BASE
#define LEN 256
#define LEN_S 256
		...
		for(j = 0; j < 32; j=j+4)
        	{
		 	...
		 	p = IORD_32DIRECT(DDR4_EMIF_ARCH_BASE,j);
                        printf("value at memory location 0x%x with offset %x is 0x%x\n",(DDR4_EMIF_ARCH_BASE),j, p);
			
			printf("Writing Data Pattern 0x0 \n");
                        IOWR_32DIRECT(DDR4_EMIF_ARCH_BASE,j,(p & 0x0));
                        p_updated = IORD_32DIRECT(DDR4_EMIF_ARCH_BASE,j);

			printf("After write: value at memory location 0x%x with offset %x is 0x%x\n",(DDR4_EMIF_ARCH_BASE),j,p_updated);
		}
