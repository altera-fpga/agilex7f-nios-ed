...
int main ()
{
	...
	lda = 201;		//2-D array that contained the matrix has a leading dimension of 201 (Read-Only)
	ldaa = 200;		//2-D array that contained the matrix has a leading dimension of 200 (Read-Only)
	cray = .056; 	//Time taken for Cray1-S to perform the same operation (Read-Only)
	n = 100;		//matrix size of 100 (Read-Only)

	printf ("Print the value of System ID \n");
	sys_id = IORD_ALTERA_AVALON_SYSID_QSYS_ID(SYSID_BASE);
	printf ("System ID from Peripheral core is 0x%X \n",sys_id);

	if(alt_timestamp_start() < 0) // starts the timestamp timer
	{
	  printf("Please enable the timestamp timer in the Nios II BSP Editor.\n");
	  exit(1);
	}
	...
	//fprintf(stderr,ROLLING);fprintf(stderr,PREC);fprintf(stderr,"Precision Linpack\n\n");
        ...
        ops = (2.0e0*(n*n*n))/3.0 + 2.0*(n*n);	//dgefa requires 2/3 n3 operations and dgesl requires n2 operations

        matgen(a,lda,n,b,&norma);
        t1 = second();
        dgefa(a,lda,n,ipvt,&info);
        time[0][0] = second() - t1;			//time in second for dgefa
        t1 = second();
        dgesl(a,lda,n,ipvt,b,0);
        time[1][0] = second() - t1;			//time in second for dgesl()
...
/*     compute a residual to verify results.  */ 
        ...
        normx = 0.0;
        for (i = 0; i < n; i++) {
            	resid = (resid > fabsf((REAL)b[i])) 
			? resid : fabsf((REAL)b[i]);
            	normx = (normx > fabsf((REAL)x[i])) 
			? normx : fabsf((REAL)x[i]);
	}
        eps = epslon((REAL)ONE);
	...
	fprintf(stderr,"      ratio\n");
        ...
        time[2][0] = total;					//time in second for dgefa() and dgesl()
        time[3][0] = ops/(1.0e3*total);		//flops per second / 1000 = kflops per second 
        time[4][0] = 2.0e3/time[3][0];		//time needed to run 2000 kflops
        time[5][0] = total/cray;			//Ratio of processor versus Cray-1 (0.056 seconds)
	...
	for (i = 0; i < ntimes; i++) {
		...
		matgen(a,lda,n,b,&norma);
		tm2 = tm2 + second() - tm;
		//tm2 = tm2 + tm - second();				//Nios V timestamp decrements: tm > second()
		dgefa(a,lda,n,ipvt,&info);
	}

		time[0][3] = (second() - t1 - tm2)/ntimes;
		//time[0][3] = (t1 - second() - tm2)/ntimes;				//Nios V timestamp decrements: t1 > second()
		t1 = second();
	...
	for (i = 0; i < ntimes; i++) {
		...
		//time[1][3] = (t1 - second())/ntimes;				//Nios V timestamp decrements: t1 > second()
	...
	if (fabsf((REAL)kf) < ONE) 
		kflops = 0;
	else {
		kflops = floor(fabsf((REAL)kf));
...
fprintf(stderr,"%11.2f%11.2f%11.2f%11.0f%11.2f%11.2f\n",   (REAL)time[0][row],
       (REAL)time[1][row], (REAL)time[2][row], (REAL)time[3][row], 
       (REAL)time[4][row], (REAL)time[5][row]);
}
...
int incx,n;
{
	...
	if(incx != 1) {
		...
		ix = 0;
		dmax = fabsf((REAL)dx[0]);
		ix = ix + incx;
		for (i = 1; i < n; i++) {
			if(fabsf((REAL)dx[ix]) > dmax)  {
				itemp = i;
				dmax = fabsf((REAL)dx[ix]);
			}
			ix = ix + incx;
		}
	}
		...
		dmax = fabsf((REAL)dx[0]);
		for (i = 1; i < n; i++) {
			if(fabsf((REAL)dx[i]) > dmax) {
				itemp = i;
				dmax = fabsf((REAL)dx[i]);
			}
		}
	}
	return (itemp);
...
*/
	...
	while (eps == ZERO) {
		b = a - ONE;
		c = b + b + b;
		eps = fabsf((REAL)(c-ONE));
	}
	return(eps*fabsf((REAL)x));
}
 
/*----------------------*/ 
int dmxpy (n1, y, n2, ldm, x, m)
REAL y[], x[], m[];
int n1, n2, ldm;
...
REAL second()
{
	REAL timeStamp = alt_timestamp();
	REAL t = ((REAL)(timeStamp))/((REAL)alt_timestamp_freq());
	return t ;
}
