 ...
 *
 * This example demonstrates the use of MicroC/OS-II running on Nios V/m.
 * In addition it is to serve as a good starting point for designs using
 * uC/OS-II and uC/TCP-IP.
 ******************************************************************************/

/* uC/TCP-IP needs to be included first to avoid redefinition errors. */
 ...
 * by the LEDLightshowTask each time it updates uesr LEDs. The
 * LEDManagementTask grabs the semaphore away from the lightshow task to toggle
 * the lightshow off, and gives up the semaphore to turn the lightshow back on.
 * The LEDManagementTask does this in response to the CMD_LEDS_LIGHTSHOW
 * command sent from the SSSSimpleSocketServerTask when the user sends a toggle
 * lightshow command over the TCP/IP socket.
 */
...
OS_STK    LEDLightshowTaskStk[TASK_STACKSIZE];
...
void SSSCreateOSDataStructs(void)
{
  ...
  * by the SSSLEDLightshowTask each time it updates user LEDs.  The LEDTask
  * grabs the semaphore away from the lightshow task to toggle the lightshow
  * off, and gives up the semaphore to turn the lightshow back on.  The LEDTask
  * does this in response to the CMD_LEDS_LIGHTSHOW command sent from the
  * SSSSimpleSocketServerTask when the user sends the toggle lightshow command
  * over the TCPIP socket.
  */
...
void SSSCreateTasks(void)
{
   ...
   error_code = OSTaskCreateExt(LEDLightshowTask,
                             NULL,
                             (void *)&LEDLightshowTaskStk[TASK_STACKSIZE-1],
                             LED_LIGHTSHOW_TASK_PRIORITY,
                             LED_LIGHTSHOW_TASK_PRIORITY,
                             LEDLightshowTaskStk,
                             TASK_STACKSIZE,
                             NULL,
...
void sss_send_menu(SSSConn* conn)
{
  ...
  tx_wr_pos += sprintf(tx_wr_pos,"Nios V/m Simple Socket Server Menu\r\n");
  tx_wr_pos += sprintf(tx_wr_pos,"==================================\r\n");
  tx_wr_pos += sprintf(tx_wr_pos,"0-3: Toggle board LEDs D0 - D3\r\n");
  tx_wr_pos += sprintf(tx_wr_pos,"S: Board LED Light Show\r\n");
...
void sss_handle_receive(SSSConn* conn)
{
  ...
  while(conn->state != CLOSE)
  {
    ...
    Mem_Move(conn->rx_buffer, conn->rx_rd_pos,
