...
#quick elaboration synthesis
elab_syn = subprocess.Popen(["quartus_syn","--quick_elab", "{}".format(qpf_dir)],stdout=subprocess.PIPE, stderr=subprocess.STDOUT,cwd=cwd_1)
for line in generate_ip.stdout:
    logfile.write(line)
elab_syn.wait()
...
#Tlg
tlg_cmd = subprocess.Popen(["quartus_tlg","--skip_quick_elaboration", "{}".format(qpf_dir)],stdout=subprocess.PIPE, stderr=subprocess.STDOUT,cwd=cwd_1)
for line in generate_ip.stdout:
    logfile.write(line)
tlg_cmd.wait()
